...
def get_file_content(path: str):
  """Returns the content of a file. If the file is larger than 1000 characters, it will only return the first 100 characters and append a "..." at the end. This is to prevent overwhelming with too much information. Consider using get_outline_from_file(path, regex) to get an overview of the file and query it for specific information."""
  return read_file_contents(codebase_root, path, None)

@mcp.tool()
def get_specific_file_content(path: str, regex: str):
  """Query the file content for the given regex. The LLM can suggest a regex, e.g. to list all functions or classes in the file. All matches of this regex against the file content will be returned. This is useful to get an overview or particular information of the file without reading the entire content. The regex should be a string, e.g. `r"def\\s+\\w+\\s*\\(.*\\):"` to match all function definitions in Python files."""
  return read_file_contents(codebase_root, path, regex)

@mcp.tool()
def find_files_with_content(regex_or_str: str):
  """Lists all file paths in the codebase where their content contain the given regex or string. The content should be a string, e.g. `r"def\\s+\\w+\\s*\\(.*\\):"` to match all function definitions in Python files."""
